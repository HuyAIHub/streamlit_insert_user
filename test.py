...
def input_people():
    ...
    with st.form(key='my_form',clear_on_submit = True):
        ...
        st.markdown("<h3 style='text-align: center; color: Black;'>Nhập Ảnh</h3>", unsafe_allow_html=True)
...
def Manament_day():
    # st.title("MANAGEMENT")
    st.markdown("<h1 style='text-align: center; color: Red;'>QUẢN LÝ CHẤM CÔNG</h1>", unsafe_allow_html=True)
    st.markdown("<hr/>", unsafe_allow_html=True)
    ...
    st.header("Kiểm Tra Chấm Công Theo Ngày")
    d = st.date_input("chọn ngày")
    # st.write('date:', d)
    print('d:',d)
    select_user = 'WITH cte_table AS(SELECT name,employee_id,MIN (time_check) as time_check FROM face_recognition.vcc_face_event WHERE CAST (time_check AS DATE) = \''+ str(d) +'\' GROUP BY (name,employee_id))\
        SELECT cte.name employee_id,cte.time_check,image_url FROM cte_table as cte INNER JOIN face_recognition.vcc_face_event using(time_check);'
...
def Manament_month():
    ...
    st.markdown("<hr/>", unsafe_allow_html=True)
    st.markdown("<h3 style='text-align: center; color: Black;'>Hãy Chọn Khoảng Thời Gian</h3>", unsafe_allow_html=True)
...
def main():
    menu = ['Thêm User','Kiểm Tra Chấm Công Ngày','Kiểm Tra Chấm Công Tháng']
